...
async def approve_join_request(company_id: UUID, request_id: UUID, request: Request) -> dict:
    user_info = await require_user(request)
...
async def deny_join_request(company_id: UUID, request_id: UUID, request: Request) -> dict:
    user_info = await require_user(request)
    admin_id = UUID(user_info["sub"])
