...
async def set_platform_role(
    user_id: UUID, body: PlatformRoleRequest, user_info: CurrentUser, session: SessionDep
) -> dict:
...
async def list_all_spaces(user_info: CurrentUser, session: SessionDep) -> dict:
...
async def update_retention_policy(
    space_id: UUID, body: RetentionPolicyRequest, user_info: CurrentUser, session: SessionDep
) -> dict:
    if not user_info.get("is_admin"):
        raise HTTPException(status_code=403, detail="Admin required")
    actor_id = UUID(user_info.get("id") or user_info.get("sub"))
...
async def bulk_reindex(user_info: CurrentUser, session: SessionDep) -> dict:
