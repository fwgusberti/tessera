...
def _make_publish_mocks(
    ...
    mock_doc_repo = AsyncMock()
    mock_doc_repo.get_by_id_for_company = AsyncMock(return_value=mock_doc)
...
class TestListDocumentsNoSpaceIdContract:
    """Contract: GET /v1/documents without space_id MUST return company-scoped docs."""

    @pytest.mark.anyio
    async def test_list_documents_no_space_id_calls_list_by_company_spaces(self):
        """Router MUST call space_repo.list_by_company and doc_repo.list_by_space_ids_for_company."""
    ...
    @pytest.mark.anyio
    async def test_list_documents_no_space_id_returns_empty_when_no_company_spaces(self):
        """When company has no spaces, MUST return empty documents list."""
        from tessera_api.routers.documents import list_documents
