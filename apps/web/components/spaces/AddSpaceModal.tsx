...
interface AddSpaceModalProps {
  ...
  onClose: () => void;
  onCreated: (created: Space) => void;
}

export function AddSpaceModal({ parentSpaceId, onClose, onCreated }: AddSpaceModalProps) {
  ...
  async function handleCreate() {
    ...
    try {
      const data = await api.post<{ space: Space }>(
        "/v1/spaces",
        parentSpaceId ? { name: trimmed, parent_space_id: parentSpaceId } : { name: trimmed }
      );
